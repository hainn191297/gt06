...
func ParseAndValidatePacket(buffer []byte) (*CONCOXPacket, error) {
	...
	if packet.StartBit[0] != 0x78 || packet.StartBit[1] != 0x78 {
	...
	if packet.StopBit[0] != 0x0D || packet.StopBit[1] != 0x0A {
...
func BuildCONCOXResponseLogin(receivedPacket *CONCOXPacket) []byte {
	...
	// Start Bit
	responsePacket = append(responsePacket, 0x78, 0x78)
	...
	// Stop Bit
	responsePacket = append(responsePacket, 0x0D, 0x0A)
...
func BuildCONCOXResponseHeartbeat(packet *CONCOXPacket) []byte {
	...
	response = append(response, 0x0D, 0x0A) // Stop Bit
